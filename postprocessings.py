...
def compute_RE_full(
    ...
    for t in tqdm(range(max_time)):
        ...
        for i in range(t):
            bf_rev = (
                broadcast_matmul(fm_r[t - i - 1][sampling_points], bf_rev)
                + random[None, t + i]
            )
...
def compute_MEGNO(
    ...
    for i in tqdm(range(max_time)):
        ...
        le_value = np.sqrt(
            np.trace(
                matrices @ np.transpose(matrices, axes=[0, 2, 1]), axis1=1, axis2=2
            )
        )
...
def frequency_map_analysis(
    ...
    for i in tqdm(range(coord.total_samples)):
        ...
        for j, t_half in enumerate(t_half_list):
            ...
            tune_2_x = birkhoff_tune(
                x_data[t_half : t_half * 2], px_data[t_half : t_half * 2]
            )
            tune_2_y = birkhoff_tune(
                y_data[t_half : t_half * 2], py_data[t_half : t_half * 2]
            )
            fma_data[j, i] = np.sqrt(
                (tune_1_x - tune_2_x) ** 2 + (tune_1_y - tune_2_y) ** 2
            )
